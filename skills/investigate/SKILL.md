...
Save to `.omc/specs/investigate-trace-{slug}.md`:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".omc/specs/investigate-trace-{slug}.md"`
...
- Save to `.omc/specs/investigate-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".omc/specs/investigate-{slug}.md"`
...
1. **Ralplan → Autopilot (Recommended)**
   ...
   - Action: Invoke `Skill("oh-my-claudecode:plan")` with `--consensus --direct` flags and the spec file path (`spec_path` from state) as context. The `--direct` flag skips the omc-plan skill's interview phase (the investigate skill already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.omc/plans/`, invoke `Skill("oh-my-claudecode:autopilot")` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `investigate spec → omc-plan --consensus --direct → autopilot execution`
...
**IMPORTANT:** On execution selection, **MUST** invoke the chosen skill via `Skill()` with explicit `spec_path`. Do NOT implement directly. The investigate skill is a requirements pipeline, not an execution agent.
...
- Use `Write` tool to save trace result to `.omc/specs/investigate-trace-{slug}.md` and final spec to `.omc/specs/investigate-{slug}.md`; use `.omc/state/` or `state_write` for ephemeral artifacts
...
```
User: /investigate "Production DAG fails intermittently on the transformation step"
...
```
User: /investigate "I want to improve our authentication flow"
...
```
User: /investigate "Fix the login bug"
...
- [ ] Phase 3 saves trace result to `.omc/specs/investigate-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.omc/specs/investigate-{slug}.md` in standard deep-interview format
...
Deep-dive's output (`.omc/specs/investigate-{slug}.md`) feeds into the standard omc pipeline:

```
/investigate "problem"
  → Trace (3 parallel lanes) + Interview (Socratic Q&A)
  → Spec: .omc/specs/investigate-{slug}.md
