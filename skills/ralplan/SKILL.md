...
6. Mark plan `pending approval` in `.omc/plans/ralplan-{slug}.md`. Output final plan.
...
The separate `/quickplan` skill has been merged into `/ralplan`. Use `/ralplan` for all planning workflows. The `--consensus` flag activates the consensus planning mode.
</Advanced>
