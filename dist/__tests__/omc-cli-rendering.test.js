...
describe('omc CLI rendering', () => {
    ...
    it('falls back to the plugin bridge when omc is unavailable but CLAUDE_PLUGIN_ROOT is set', () => {
        ...
        expect(formatOmcCliInvocation('auto-improve --mission "m"', { omcAvailable: false, env }))
            .toBe('node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs auto-improve --mission "m"');
    });
    it('rewrites inline and list-form omc commands for plugin installs', () => {
        const env = { CLAUDE_PLUGIN_ROOT: '/tmp/plugin-root' };
        const input = [
            'Run `omc auto-improve --mission "m" --eval "e"`.',
        ...
        expect(output).toContain('`node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs auto-improve --mission "m" --eval "e"`');
