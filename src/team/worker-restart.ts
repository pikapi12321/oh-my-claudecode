...
import type { BridgeConfig, McpWorkerMember } from './types-lite.js';
...
export function shouldRestart(
  ...
  if (!state) {
    // No prior restart state — OK to restart immediately.
    return 0;
  }

  if (state.restartCount >= policy.maxRestarts) {
    return null; // Exhausted — do not restart.
  }
  ...
  // Enforce cooldown timing: check elapsed time since last restart.
  const elapsed = Date.now() - new Date(state.lastRestartAt).getTime();
  if (elapsed < backoff) {
    return backoff - Math.floor(elapsed); // Remaining cooldown ms.
  }

  return 0; // Cooldown elapsed — OK to restart.
}
...
export function synthesizeBridgeConfig(
  ...
  const raw = worker.agentType.replace('mcp-', '').toLowerCase();
  const provider: 'codex' | 'gemini' = raw === 'codex' || raw === 'gemini' ? raw : 'codex';
  return {
